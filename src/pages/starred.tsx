...
export default function EmailStarred() {
  ...
  useEffect(() => {
    setStarredDrafts(starredData?.map((email) => email.id) || []);
  }, [starredData]);
          ...
          <div className="ml-2 flex items-center gap-2">
            <div className="group relative rounded-full p-2">
              <Checkbox
                ...
                checked={selectedDrafts.length === starredData?.length}
                onClick={() => {
                  if (selectedDrafts.length === starredData?.length) {
                    setSelectedDrafts([]);
                  } else {
                    setSelectedDrafts(
                      starredData?.map((email) => email.id) || [],
                    );
                  }
                }}
              />
        ...
        <div className="flex-1 overflow-auto">
          {isLoading || isFetching || isFetchingNextPage ? (
            ...
            <div className="flex h-full flex-col items-center justify-center">
              ...
              <p className="mt-4 text-lg text-gray-500">No starred emails found</p>
            </div>
          ) : (
            <Table>
              <TableBody>
                {starredData.map((email) => {
                  ...
                  const isSelected = selectedDrafts.includes(email.id);
                  const isStarred = starredDrafts.includes(email.id);
                  ...
                  return (
                    ...
                    >
                      <TableCell className="flex w-60 items-center gap-1 p-1 pl-4 hover:shadow-md">
                        ...
                        <span className="text-gray-800">{email.sender?.name}</span>
                      </TableCell>
